...
def vM_Projection_Run(T,kappa_phi,z=None,alpha=0,dy=None,dy2=None,kappa_y=0,
    ...
    for i in range(1,n_point):
        ...
        # Update with 2nd meausurement
        if dy2 is not None: 
            [mu[i],kappa[i]] = vM_Projection(mu[i],kappa[i],
                                            kappa_phi, #diffusion
                                            z=0,alpha=alpha, # direct obs
                                            dy=dy2[i],kappa_y=kappa_y, #relative heading info
                                            dt=dt)
    return mu, kappa
